...
<body>
	...
	<form action='./updateCtr.do' method='post' enctype="multipart/form-data">
			...
			<c:choose>
				...
				<c:otherwise>
					<c:forEach var="row" items="${fileList}" varStatus="obj">
						...
						<input type="file" id="file_${obj_index}" name="file_${obj_index}">
							(${row.FILE_SIZE}kb)
						<a href="#this" id="delete_${obj_index}">삭제</a><br>
					</c:forEach>
				</c:otherwise>
			</c:choose>
			</div>
		...
		가입일: <fmt:formatDate pattern="yyyy-MM-dd hh:mm:ss" value="${requestScope.memberDto.createDate}"/><br>
